...
if (isset($_FILES['image']) && $_FILES['image']['error'] == 0) {
    ...
    // ใช้ภาพใหม่
    $sql = "UPDATE advice SET name='$name', details='$details', image='$newImageName', updated_at=NOW() WHERE id=$id";
} else {
    // ไม่อัปโหลดภาพใหม่ → ไม่อัปเดตฟิลด์ image
    $sql = "UPDATE advice SET name='$name', details='$details', updated_at=NOW() WHERE id=$id";
}

if (mysqli_query($conn, $sql)) {
    header("Location: advice.php?success=2");
    exit();
} else {
